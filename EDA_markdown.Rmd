...
```{r Sample  stattistics}
#Step2 - Sample Statistics
library(psych)
...
#Step3 - Data Visualization interesting hidden insights#
#DOT CHART - HOW TO MAKE FONT VISIBLE
dotchart (cereals_data$rating,cereals_data$name,
...
count_mfr
View(count_mfr)
data_frame=data.frame(count_mfr)
class(data_frame)
View(data_frame)
library(treemap)
treemap(data_frame,index = c("Var1"),vSize = "Freq",title = "Number of Products by Manufacturer")
...
```{r}
data1=sum(cereals_data$protein)        #protein
data1                                 
...
```{r Cluster Analysis}
dist_data=dist(numeric_data1,method="euclidean")    
head(dist_data)
fviz_dist(dist_data)
